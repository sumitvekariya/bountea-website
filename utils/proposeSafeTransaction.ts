...
import { ethers, getBytes, keccak256, toUtf8Bytes, AbiCoder, ZeroAddress } from "ethers"
...
export const proposeTransaction = async (
  ...
  try {
    ...
    const signature = (
      await mteWallet.signMessage(getBytes(hashToSign))
    )
      .replace(/1b$/, "1f")
      .replace(/1c$/, "20")
...
const formatResliceData = (
  ...
  keccak256(toUtf8Bytes("reslice(uint256,address[],int32[])"))
    .slice(0, 10) +
